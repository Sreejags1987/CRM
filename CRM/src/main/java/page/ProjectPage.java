...
public class ProjectPage {
	...
	public String doSearchProject(String searchvalue) {
		uwait.waitForVisibility(searchfield);
		searchfield.clear();
		uwait.waitForVisibility(searchfield);
		searchfield.sendKeys(searchvalue);
		By locator=By.xpath("//table[@id='project-table']//tbody//td[2]//a[contains(text(),'"+searchvalue+"')]");
		uwait.waitForVisibility(locator);
		List<WebElement> notetable=driver.findElements(By.xpath("//table[@id='project-table']//tbody//td[2]//a[contains(text(),'"+searchvalue+"')]"));
		uwait.waitForVisibility(notetable);
		int row=eutility.getTableDataRowCount(notetable,searchvalue );
		String message="";
		if(row!=0) 
		{
